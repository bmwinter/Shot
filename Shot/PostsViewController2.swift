...
class Posts2ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func nextImage() {
        ...
        // decrement index and load next image
        if (currentImageIndex > 0) {
            currentImageIndex = currentImageIndex - 1
            loadNextImage()
        } else {
            
            // reset index and imageURL array
            currentImageIndex = self.imageURLS.count - 1
            imageURLS = [String]()
            displayCameraAlert()
        }
